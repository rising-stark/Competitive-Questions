...
void update(int arr[], int si, int ss, int se, int i, int k){
	if(ss==se){
		st[si]=k;
		return;
	}
	int mid=ss+(se-ss)/2;
	if(i<=mid){
		update(arr, 2*si, ss, mid, i, k);
	}else{
		update(arr, 2*si+1, mid+1, se, i, k);
	}
	st[si]= combine(st[2*si], st[2*si+1]);
}

ll query(int si, int ss, int se, int i, int j){
	if(i>se || j<ss){
		return 0;
	}
	if(i<=ss && se<=j){
	...
	int mid=ss+(se-ss)/2;
	return combine(query(2*si, ss, mid, i, j), query(2*si+1, mid+1, se, i, j));
}
...
void solve(){
	...
	while(q--){
		...
		i++;
		if(t==1){
			update(arr, 1, 1, n, i, j);
		}else{
			ll p=query(1, 1, n, i, j);
			cout<<p<<endl;
		}
	}
}