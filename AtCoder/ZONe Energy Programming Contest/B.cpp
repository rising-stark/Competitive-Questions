...
inline void yes(bool b) { cout << (b ? "Yes" : "No") << endl; exit(0); }
inline void yes(int b) { cout << (b==1 ? "Yes" : "No") << endl; exit(0); }
...
void solve(){
	...
	for(i=0;i<n;i++){
		int flag = 1;
		double c = (h-arr[i].ss)/(d-arr[i].ff);
		for(j=0;j<n;j++){
			/*if(i==j)
				continue;
			*/double y = arr[i].ss + (c*(arr[j].ff-arr[i].ff));
			double y1 = arr[j].ss;
			if(y1-eps>y){
				flag=0;
				break;
		...
		if(flag){
			ans = min(ans, max(0.0, arr[i].ss - c*arr[i].ff));
		}
	}
	cout<<ans<<endl;
}
...
Can also be done using binary search over the minimum possible height.
But the task is tedious, So it is not implemented here. Wi
void solve(){
	ll i, j, n, d, h;
	cin>>n>>d>>h;
	vll arr(n);
	cin>>arr;
	double low = 0.0, high = 1000.0, ans = high;
	while((high-low) > eps){
