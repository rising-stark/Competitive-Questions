...
void brute(ll a, ll n){
	...
	for(int i=a;i<=n;i++){
		...
		if(s.find("62")!=string::npos || s.find("4")!=string::npos){
			ans--;
		}
	}
	cout<<ans<<endl;
...
int main(){
	...
	if(stress == 1){
		...
		for(int i=0;i<t;i++){genTestCases();}cout<<"0 0"<<endl;cout.flush();
		freopen("C:/Users/ujjwa/Desktop/Practice/code/Competitive-Questions/input.txt", "r", stdin);
...
ll recur(string &arr, int i, int prev, int t){
	int n = (int)arr.size();
	if(i==0)
		return 1;
	ll &ret = dp[i][prev];
...
void solve(ll a, ll b){
	...
	ll ans=recur(bs, bs.size(), 0, 1)-recur(as, as.size(), 0, 1);
	cout<<ans<<endl;
}