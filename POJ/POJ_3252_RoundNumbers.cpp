...
int main(){
	...
	if(stress == 1){
		...
		for(int i=0;i<t;i++){ll a, b;
		cin>>a>>b;brute();}cout.flush();
		#ifndef ONLINE_JUDGE
		freopen("C:/Users/ujjwa/Desktop/Practice/code/Competitive-Questions/input.txt", "r", stdin);
		freopen("C:/Users/ujjwa/Desktop/Practice/code/Competitive-Questions/output.txt", "w", stdout);
		#endif
...
ll recur(string &arr, int i, int ones, int zero, int t, int lead){
	if(ones > 16){
		return 0;
	}
	int n=(int)arr.size();
	...
	ll &ret = dp[i][ones][zero];
	if(!t && !lead && ret!=-1){
		return ret;
	}
	int ans=0, lmt = t?arr[n-i]-'0':1;
	for(int j=0;j<=lmt;j++){
		if(lead && j==0)
			ans+=recur(arr, i-1, ones, zero, t&(j==lmt), 1);
		else
			ans+=recur(arr, i-1, ones+(j==1), zero+(j==0), t&(j==lmt), 0);
	}
	return (t || lead)?ans:ret=ans;
}
ll solve(ll b){
	string bs = toBin(b);
	ll ans = recur(bs, bs.size(), 0, 0, 1, 1);
	return ans;
}