...
// DP memory efficient since the result depends only on previous row
void solve(int m, int n){
	...
	for(i=0;i<=m;i++){
		for(j=0;j<=n;j++){
			...
			dp[i&1][j]=dp[i&1][j-1];
			ll x = (j<arr[i-1].first)?0:dp[(i+1)&1][j-arr[i-1].first];
			ll y = (j<arr[i-1].second+1)?0:dp[(i+1)&1][j-arr[i-1].second-1];
			dp[i&1][j]+=x-y;
		}
	}
	cout<<dp[m&1][n]-dp[m&1][n-1]<<endl;
}

/*
DP made faster using prefix sums
void solve(int m, int n){
	ll i, j;
	vpii arr(m);
	cin>>arr;
	...
	for(i=0;i<=m;i++){
		for(j=0;j<=n;j++){
			...
			dp[i][j]=dp[i][j-1];
			ll x = (j<arr[i-1].first)?0:dp[i-1][j-arr[i-1].first];
			ll y = (j<arr[i-1].second+1)?0:dp[i-1][j-arr[i-1].second-1];
			dp[i][j]+=x-y;
		}
	}
	cout<<dp[m][n]-dp[m][n-1]<<endl;
}
*/
...
void solve(int m, int n){
	...
	for(i=1;i<=m;i++){
		for(j=0;j<=n;j++){
			for(int k = arr[i-1].first;k<=arr[i-1].second;k++){
				if(j>=k){
					dp[i][j]+=dp[i-1][j-k];
				}
			}
