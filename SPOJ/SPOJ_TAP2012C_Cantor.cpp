...
int main(){
	...
	while(1){
		ll a, b;
		cin>>a>>b>>base>>s;
		if(a==-1 || b==-1 || base==-1 || s[0]=='*')
			return 0;
		cout<<(solve(toBase(b, base)) - solve(toBase(a-1, base)))<<endl;
	}
	if(stress==1){compare();}
	return 0;
}
ll recur(string &arr, int i, int t, int lead){
	if(i==0)
		return 1;
	int n = (int)arr.size();
	ll &ret = dp[i];
	if(!t && !lead && ret!=-1)
		return ret;
	int lmt = t?arr[n-i]-'0':(base-1);
	ll ans=0;
	for(int j=0;j<=lmt;j++){
		if(lead && j==0){
			ans+=recur(arr, i-1, t&(j==lmt), 1);
		}else{
			if(s[j]=='S'){
