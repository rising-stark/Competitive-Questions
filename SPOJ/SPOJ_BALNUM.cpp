...
inline string toBin(ll a){string s="";if(!a)return"0"; while(a>0){s=to_string(a&1)+s;a>>=1;}return s;}
...
ll dp[20][1<<10][32];
/*
dp[i][j][k] = represents the ith index,
              j is 10 bits to keep the count of digits but since 
              actual counts don't matter, its parity is enough
              k is whether an even digit has occured atleast once
*/
...
ll recur(string &arr, int i, int c, int d, int t, int lead){
	if(i==0){
		for(int k=0;k<=9;k++){
			int occ = (c & (1<<k))>0;
			if(occ>0 && k&1 && occ&1){
	...
	for(int j=0;j<=lmt;j++){
		if(lead && j==0){
			ans+=recur(arr, i-1, c, d, t&(j==lmt), 1);
		}else{
			ans+=recur(arr, i-1, c^(1<<j), (j&1)?(d):(d|(1<<(j/2))), t&(j==lmt), 0);
		}
	}
...
void solve(){
	...
	cout<<(recur(b, b.size(), 0, 0, 1, 1)-recur(as, as.size(), 0, 0, 1, 1))<<endl;
}