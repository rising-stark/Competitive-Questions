...
int d;
ll recur(string &arr, int i, int t, int c){
	int n = (int)arr.size();
	if(i==0){
		return c;
	}
...
void solve(ll a, ll b){
	for(d=0;d<=9;d++){
		...
		ll ans = recur(bs, bs.size(), 1, 0);
		ll ans1=0;
		string as;
		if(a>=1){
			as = to_string(a-1);
			ans1=recur(as, as.size(), 1, 0);
		}
		if(d==0 && as.size()<bs.size()){
			ll ret = (int)bs.size()-(int)as.size();
			for(int i=1;i<bs.size();i++){
				ret+=(9*pow(10,bs.size()-i-1))*i;
			}
			for(int i=1;i<as.size();i++){
				ret-=(9*pow(10,as.size()-i-1))*i;
			}
			cout<<ans-ans1-ret<<" ";
		}else{
			cout<<ans-ans1<<" ";
