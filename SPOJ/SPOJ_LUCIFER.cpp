...
ll dp[20][180][180][2];
ll recur(string &arr, int i, ll odd, ll even, int t){
	int n = (int)arr.size();
	if(i==n){
		if(n&1){
			return isprime(odd-even);
		}else{
			return isprime(even-odd);
		}
	}
	if(dp[i][odd][even][t]!=-1){
		return dp[i][odd][even][t];
	}
	int lmt = t?arr[i]-'0':9;
	ll ans=0;
	for(int j=0;j<=lmt;j++){
		if(i&1){
			ans+=recur(arr, i+1, odd+j, even, t&(j==lmt));
		}else{
			ans+=recur(arr, i+1, odd, even+j, t&(j==lmt));
		}
	}
	return dp[i][odd][even][t]=ans;
}
void solve(){
	ll a, b;
	cin>>a>>b;
	...
	ans = recur(bs, 0, 0, 0, 1);
	if(a>1){
		string as = to_string(a-1);
		mem;
		ans1 = recur(as, 0, 0, 0, 1);
	}
	cout<<ans-ans1<<endl;
}
