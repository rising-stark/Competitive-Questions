...
ll dp[20][200][200];
ll recur(string &arr, int i, int odd, int even, int t){
	if(i==0)
		return even>odd;
	ll &ret = dp[i][even][odd];
	if(!t && ret!=-1)
		return ret;
	int n = (int)arr.size();
	...
	ll ans=0;
	for(int j=0;j<=lmt;j++){
		ans+=recur(arr, i-1, odd+j*(j&1), even+j*(!(j&1)), t&(j==lmt));
	}
...
void solve(){
	...
	mem;
	cout<<(recur(b, b.size(), 0, 0, 1) - recur(as, as.size(), 0, 0, 1));
}