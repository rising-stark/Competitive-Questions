...
ll dp[20][20][200];
ll recur(string &arr, int i, int k, int sum, int t){
	if(k<0 || sum<0)
		return 0;
	if(i==0)
		return (k==0)&&(sum==0);
	...
	ll ans=0;
	for(int j=0;j<=lmt;j++){
		ans+=recur(arr, i-1, k-(j!=0), sum-j, t&(j==lmt));
	}
...
void solve(){
	ll i, n ,k;
	cin>>n>>k;
	string bs=to_string(n);
	/*
	Since we need numbers of numbers with distinct odd sum,
	there are atmost 200 differet possible digit sums.
	We can brute force that and find if there exits numbers with
	exactly k non-zero digits and that particular sum
	*/
	ll ans=0;
	mem;
	for(i=1;i<=200;i+=2){ // we iterate only odd digits sums so i+=2
		ans+=recur(bs, bs.size(), k, i, 1)>0;
	}
	cout<<ans<<endl;
}