...
/*
This problem requires us to find the shortest of all paths between a src ad dest which have even number of edges.
We could start thinking about it using a 2D DP table of dp[n][n*(n-1)/2] (n*(n-1)/2 = max edges) where in dp[i][j],
i represents the shortest distance from src to ith vertex using exactly j edges. Using this technique we can also
solve the problem by taking the min over all even j, but this dp table is too big in size and initializing it with
inf every testcase requires a lot of time.

But after you have thought this you can think that you only need to know whether the parity of edge count and the
actual edge does not matter. So, it will be enough to initialize the DP table using dp[n][2] only.
Now whenever we add an edge we change the parity using !depth.
And finally we take the minimum of dp[dest][0]
*/
int main(){
...
void solve(int n){
	...
	set<pair<ll, pll>> q;
	q.insert({0, {src, 0}}); // cost, vertex, #edges
	while(!q.empty()){
		pair<ll, pll> temp = *q.begin();
		q.erase(q.begin());
		...
		if(cost == inf)
			break;
		/*if(v == dest){
			// This is omitted because there can be a cycle which makes the edge count even
			// but the normal path is not even
			// Try test case mentioned below
			continue;
		}*/
		for(auto e:adj[v]){
			if(dp[v][depth]!=inf && dp[e.ff][!depth] > dp[v][depth] + e.ss){
				q.erase({dp[e.ff][!depth], {e.ff, !depth}});
				dp[e.ff][!depth] = dp[v][depth] + e.ss;
				q.insert({dp[e.ff][!depth], {e.ff, !depth}});
			}
		}
	}
	ll ans = dp[dest][0];
...
/*
The test case that fails if "if(v==dest)"" is uncommented
