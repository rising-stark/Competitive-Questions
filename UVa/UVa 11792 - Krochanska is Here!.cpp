...
/*
This is straight-forward BFS.
Here we have to first find the intersection of the station lists of different lines.
Let's say we store the stations that are common to atleast 2 different lines in set s1
Then use each of the stations in s1 as the source to find the distance to other stations and choose the minimum
Also note that although we need to find the minimum average of the sum of distances, just the minimum sum would suffice
because the denominator would be same for all the different sources. Also, taking the average could lead to WA
becuase when dividing the sum with size, it may lead to interger truncation and if you choose to use double then
floating point errors due to rounding off might give different answer. So, using just sum would be better choice.
*/
...
void solve(){
	...
	n++;
	vi adj[n];
	set<int> s, s1; // s is universal set and s1 is set of all duplicate or "important" stations
	forj{
		int u, v;
		cin>>u;
		while(cin>>v && v){
