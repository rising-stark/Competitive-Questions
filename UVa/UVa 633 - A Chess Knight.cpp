...
tioc vector<T>& v){for(T i:v){os<<i<<" ";}return os<<endl;}
...
#define piii pair<pii, pii> // {{type, distance}, {x, y}}
...
void solve(){
	...
	while(1){
		...
		src.ff = {0, 0};
		vpii obs;
		vi dx{-2,-1, 1, 2, 2, 1, -1, -2}; // Clockwise Knight Direction. Start at NE
		...
		vis[3][src.ss.ff][src.ss.ss]=1;
		vis[1][src.ss.ff][src.ss.ss]=1;
		vis[2][src.ss.ff][src.ss.ss]=1;
		//w(src);
		queue<piii> q;
		q.push(src);
		ll ans = INT_MAX;
		while(!q.empty()){
			...
			int t = temp.ff.ff; // type
			int d = temp.ff.ss+1; // dist
			int x = temp.ss.ff; // x
			int y = temp.ss.ss; // y
			if(t!=1){
				for(i=0;i<8;i++){
					int newx=x+dx[i];
					int newy=y+dy[i];
					if(newx>=1 && newx<=(2*n) && newy>=1 && newy<=(2*n) && !vis[1][newx][newy]){
						vis[1][newx][newy]=1;
						//w(newx, newy, d);
						q.push({{1, d},{newx, newy}});
					}
				}
			}
			if(t!=2){
				for(int a:{-2, 2}){
					for(int b:{-2, 2}){
						int newx=x+a;
						int newy=y+b;
						if(newx >=1 && newx<=(2*n) && newy>=1 && newy<=(2*n) && !vis[2][newx][newy]){
							vis[2][newx][newy]=1;
							//w(newx, newy, d);
							q.push({{2, d},{newx, newy}});
						}
					}
				}
			}
