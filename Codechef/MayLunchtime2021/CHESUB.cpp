...
ll dp[100001][101][2];
ll recur(vll &arr, int i, int n, int j, int cur, int k){
	int r = n-i;
	int s = k-j+1;
	if(r<s)
		return -inf;
	...
	ll a = 0;
	if(cur==0){
		a = recur(arr, i+1, n, j, 0, k);
	}else{
		a = recur(arr, i+1, n, j+1, 0, k);
	}
	ll b = arr[i]*j + recur(arr, i+1, n, j, 1, k);
	ll c = arr[i]*j + recur(arr, i+1, n, j+1, 0, k);
	return ret=maxx(a, b, c);
}
