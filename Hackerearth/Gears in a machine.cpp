...
void dfs(vi adj[], int u, int c, vi &com, vi &arr, vi &vis, vi &ans, int t, bool &flag){
	ans[u] = t;
	vis[u] = 1;
	com[u] = c;
	for(auto v:adj[u]){
		if(vis[v]==1){
			if(arr[v]==arr[u] && ans[v]==ans[u]){
				flag=0;
			}
			if(arr[v]!=arr[u] && ans[v]!=ans[u]){
				flag=0;
			...
			dfs(adj, v, c, com, arr, vis, ans, arr[v]==arr[u]?!t:t, flag);
		}
	}
}
...
void solve(){
	...
	for(i=0;i<n;i++){
		if(!vis[i]){
			bool flag = 1;
			dfs(adj, i, c, com, arr, vis, ans, 1, flag);
			c++;
			fl.pb(flag);
		}
	}
	while(q--){
		...
		cin>>a>>b>>c>>d;
		a--;b--;
		if(!fl[com[a]] || !fl[com[b]]){
			no();
		}else{
			if(com[a]==com[b]){
				if((ans[a]==ans[b] && c==d) || (ans[a]!=ans[b] && c!=d))
					yes();
				else
					no();
			}else{
