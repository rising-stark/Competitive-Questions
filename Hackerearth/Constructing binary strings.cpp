...
ll ncr(ll n, ll k){
	...
	if(k==1)
		return n;
	return (((fact[n]*1LL*cinv[k])%mod)*cinv[n-k])%mod;
}
void solve(){
	ll x, y;
	cin>>x>>y;
	...
	vll a = toBase(x+y, mod);
	vll b = toBase(y, mod);
	int i, n = sz(b);
	ll ans=(((y+1-x)%mod)*binexp(y+1, mod-2, mod))%mod;
	fori{
		ans=(ans*ncr(a[i], b[i]))%mod;
	}
	cout<<ans;ce
}