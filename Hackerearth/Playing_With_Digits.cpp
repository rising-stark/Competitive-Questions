...
ll recur(string &arr, int i, int sum1, int sum2, int t, int d){
	if(i==0)
		return isprime(sum1)&&sum2==0;
	...
	ll ans=0;
	for(int j=0;j<=lmt;j++){
		ans=(ans + recur(arr, i-1, sum1+j, (sum2*10+j)%d, t&(j==lmt), d));
	}
...
void solve(){
	...
	if(k<=10000){
		ans=recur(bs, bs.size(), 0, 0, 1, k)-recur(as, as.size(), 0, 0, 1, k);
	}else{
		for(i=0;i<=b;i+=k){
			if(i>=a){
				ans+=isprime(digsum(i));
			}
		}
	}
	cout<<ans<<endl;
